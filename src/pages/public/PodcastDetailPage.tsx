...
import { Headphones, RefreshCw, Play, List, Share2, AlertTriangle, Loader2 } from 'lucide-react';
...
const PodcastDetailPage: React.FC = () => {
  ...
  const sortedEpisodes = useMemo(() => {
    ...
    return [...podcast.Episodes].sort((a: EpisodeTypeFromDB, b: EpisodeTypeFromDB) => {
  ...
  const fetchPodcastAndEpisodes = async () => {
    ...
    try {
      ...
      let { data: podcastData, error: podcastError } = await podcastQuery.single();
      ...
      if ((podcastError || !podcastData) && !isUUID(slug)) {
        console.log(`Podcast not found by slug '${slug}', trying by name match...`);
        ...
        const { data: nameMatchData, error: nameMatchError } = await nameMatchQuery;
        ...
        if (nameMatchData && !nameMatchError) {
          ...
          if (nameMatchError && nameMatchError.code !== 'PGRST116') {
            ...
            console.error('Error fetching podcast by name match:', nameMatchError);
          }
        }
      }

      if (podcastError || !podcastData) {
        console.error('Error fetching podcast or podcast not found:', podcastError?.message);
        if (podcastError?.code === 'PGRST116' || !podcastData) {
          setError('Podcast not found. It might have been moved, deleted, or the link is incorrect.');
        } else {
          setError(`Failed to fetch podcast: ${podcastError?.message || 'Unknown error'}`);
        }
        setPodcast(null);
        setLoading(false);
        return;
      ...
      if (episodesError) {
        ...
        setError(`Podcast data loaded, but failed to fetch episodes: ${episodesError.message}. You can try refreshing episodes.`);
      } else {
        finalPodcastWithEpisodes = { ...currentPodcast, Episodes: episodesData || [] };
      }
      ...
      if (currentPodcast.category_id) {
        ...
        if (recommendedError) {
          console.error('Error fetching recommended podcasts:', recommendedError.message);
        } else {
          setRecommendedPodcasts(recommended as PodcastType[] || []);
        }
      } else if (currentPodcast.category) { // Fallback if category_id is null but category name exists
          const { data: recommended, error: recommendedError } = await supabase
          .from('podcasts')
          ...
          .limit(4);
          if (recommendedError) {
            console.error('Error fetching recommended podcasts by category name:', recommendedError.message);
          } else {
            setRecommendedPodcasts(recommended as PodcastType[] || []);
          }
      }
      ...
      console.error('Unexpected error in fetchPodcastAndEpisodes:', err.message);
    ...
    return (
      <PageLayout title="Error">
        <div className="container mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-10 text-center">
          ...
          <Link to="/podcasts" className="bg-accent-500 hover:bg-accent-600 text-white font-semibold py-2 px-4 rounded transition-colors">
            Back to Podcasts
          </Link>
        </div>
      </PageLayout>
  ...
  if (!podcast) { 
    return (
      <PageLayout title="Podcast Not Found">
        <div className="container mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-10 text-center">
          <h1 className="text-3xl font-bold text-white mb-2">Podcast Not Found</h1>
          <p className="text-lg text-gray-300 mb-6">The podcast you are looking for does not exist or has been moved.</p>
          <Link to="/podcasts" className="bg-accent-500 hover:bg-accent-600 text-white font-semibold py-2 px-4 rounded transition-colors">
            Back to Podcasts
          </Link>
        </div>
    ...
    <PageLayout title={podcast.name || 'Podcast Details'}>
        ...
        <div className="container mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-10">
          <div className="grid grid-cols-12 gap-8">
            ...
            <div className="col-span-12 md:col-span-8 lg:col-span-9">
              <h1 className="text-4xl font-bold mb-2 text-white">{podcast.name}</h1>
              <p className="text-lg text-gray-400 mb-1">By {podcast.author || 'Unknown Author'}</p>
              ...
              {podcast.description && (
                <div ref={descriptionRef} className="prose prose-invert max-w-none text-gray-300 mb-6" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(podcast.description) }} />
              )}
              ...
              <div className="flex items-center gap-4 mb-8">
                ...
                >
                  {refreshingEpisodes ? <Loader2 className="animate-spin" size={18}/> : <RefreshCw size={18}/>}
              ...
              <div ref={episodesRef}>
                <h2 className="text-2xl font-semibold mb-4 text-white border-b border-dark-700 pb-2">Episodes</h2>
                {error && !loading && (
      ...
      <div className="container mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-10">
        ...
        {recommendedPodcasts.length > 0 && (
          <div>
            <h2 className="text-2xl font-bold mb-6 border-b border-dark-700 pb-3 text-white">You Might Also Like</h2>
          ...
          <div className="bg-dark-800 rounded-lg shadow-xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-4 max-h-96 overflow-y-auto">
              <PodcastPlaylist
                ...
                currentEpisode={playerStoreCurrentEpisode ? {
                  ...playerStoreCurrentEpisode,
                  ...
                  duration: String(playerStoreCurrentEpisode.duration || 0), 
                  // @ts-expect-error - Assuming playerStoreCurrentEpisode might not have these yet, will be fixed by updating Episode type
                  published_at: playerStoreCurrentEpisode.pub_date || new Date().toISOString(),
              } : undefined} 
              />
            </div>
          </div>
