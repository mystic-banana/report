...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  try {
    ...
    const fetchPromise = supabase
      .from('profiles')
      .select('name, avatar_url, is_premium, preferences, saved_content, is_admin')
      .eq('id', userId)
      .single();
  ...
  login: async (email, password) => {
    ...
    try {
      ...
      console.log('[AuthStore] signInWithPassword result:', { data, signInError });

      if (signInError) throw signInError;

      if (data.user && data.session) {
        console.log('[AuthStore] Login successful, fetching profile for user:', data.user.id);
        ...
        set({ user: appUser, session: data.session, isAuthenticated: true, isLoading: false, error: null });
        console.log('[AuthStore] Login state updated, user authenticated.');
      } else {
        console.error('[AuthStore] Login: No user data/session received after signInWithPassword.');
        throw new Error('Login successful but no user data received.');
      }
    } catch (error: any) {
      console.error('[AuthStore] Login error caught:', error);
      set({ error: error.message || 'Invalid email or password', isLoading: false, user: null, session: null, isAuthenticated: false });
    }
  },
  ...
  register: async (name, email, password) => {
    ...
    try {
      ...
      if (data.user && data.session) {
        console.log('[AuthStore] Registration successful, fetching profile for user:', data.user.id);
        const profile = await get().fetchUserProfile(data.user.id);
        console.log('[AuthStore] Profile fetched in registration:', profile);
        const appUser = mapSupabaseUserToAppUser(data.user, profile);
        set({ user: appUser, session: data.session, isAuthenticated: true, isLoading: false, error: null });
        console.log('[AuthStore] Registration state updated, user authenticated.');
      } else if (data.user && !data.session) {
        console.log('[AuthStore] Registration successful, but no session received.');
        set({ isLoading: false, error: null });
        alert('Registration successful! Please check your email to confirm your account.');
      } else {
        console.error('[AuthStore] Registration: No user data/session received after signUp.');
        throw new Error('Registration successful but no user data received.');
      }
    } catch (error: any) {
      console.error('[AuthStore] Registration error caught:', error);
      set({ error: error.message || 'Registration failed. Please try again.', isLoading: false, user: null, session: null, isAuthenticated: false });
    }
  },
  ...
  logout: async () => {
    ...
    if (error) {
      ...
      set({ user: null, session: null, isAuthenticated: false, isLoading: false, error: null, justConfirmedEmailAndSignedIn: false });
    }
  },
  ...
  initializeAuth: () => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      try {
        if (session && session.user) {
          set({ isLoading: true }); // Indicate loading has started
          const wasAuthenticatedBefore = get().isAuthenticated;
          console.log('[AuthStore] initializeAuth: session exists, wasAuthenticatedBefore:', wasAuthenticatedBefore, 'email_confirmed_at:', session.user.email_confirmed_at);
          ...
          console.log('[AuthStore] initializeAuth: profile fetched:', profile);
          const appUser = mapSupabaseUserToAppUser(session.user, profile);
          const shouldRedirect = !wasAuthenticatedBefore && !!session.user.email_confirmed_at;
          console.log('[AuthStore] initializeAuth: shouldRedirect flag:', shouldRedirect);
          ...
          set({
            ...
            isLoading: false,
            justConfirmedEmailAndSignedIn: shouldRedirect,
          });
        } else {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        console.log('[AuthStore] onAuthStateChange: wasAuthenticatedBefore:', wasAuthenticatedBefore);

        // Set a timeout to prevent hanging during auth state changes
        const stateChangeTimeout = setTimeout(() => {
          console.log('[AuthStore] onAuthStateChange timeout reached for event:', event);
          set({ isLoading: false });
        }, 5000);
        ...
        try {
          if (event === 'SIGNED_IN' && session && session.user) {
            ...
            console.log('[AuthStore] onAuthStateChange: SIGNED_IN event, email_confirmed_at:', session.user.email_confirmed_at);
            
            // Set a timeout for profile fetching
            const profilePromise = get().fetchUserProfile(session.user.id);
            const timeoutPromise = new Promise<null>((_, reject) => {
              setTimeout(() => {
                reject(new Error('Profile fetch timed out during auth state change'));
              }, 3000);
            });
            ...
            const profile = await Promise.race([profilePromise, timeoutPromise])
              .catch(err => {
                console.error('[AuthStore] Error fetching profile during auth state change:', err);
                return null;
              });
            
            console.log('[AuthStore] onAuthStateChange SIGNED_IN: profile fetched:', profile ? 'exists' : 'null');
            const appUser = mapSupabaseUserToAppUser(session.user, profile);
            const isInitialSignInAfterConfirmation = !wasAuthenticatedBefore && !!session.user.email_confirmed_at;
            console.log('[AuthStore] onAuthStateChange SIGNED_IN: isInitialSignInAfterConfirmation flag:', isInitialSignInAfterConfirmation);
            ...
            set({ session: null, user: null, isAuthenticated: false, isLoading: false, error: null, justConfirmedEmailAndSignedIn: false });
          } else if (event === 'USER_UPDATED' && session && session.user) {
            set({ isLoading: true, error: null }); // Indicate loading has started
            console.log('[AuthStore] onAuthStateChange: USER_UPDATED event, email_confirmed_at:', session.user.email_confirmed_at);
            ...
            const profile = await Promise.race([profilePromise, timeoutPromise])
              .catch(err => {
                console.error('[AuthStore] Error fetching profile during user update:', err);
                return null;
              });
            
            console.log('[AuthStore] onAuthStateChange USER_UPDATED: profile fetched:', profile ? 'exists' : 'null');
            const appUser = mapSupabaseUserToAppUser(session.user, profile);
            set({ session, user: appUser, isAuthenticated: true, isLoading: false, error: null });
