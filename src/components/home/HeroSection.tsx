  ...
  useEffect(() => {
    const fetchHeroArticles = async () => {
      try {
        ...
        if (!articlesData || articlesData.length === 0) {
          ...
          setSlides([
            ...
            },
            {
              ...
              image: 'https://images.pexels.com/photos/5417678/pexels-photo-5417678.jpeg?auto=compress&cs=tinysrgb&w=1600',
              category: 'Astrology',
        ...
        const authorAvatar = '';
        const defaultImage = 'https://images.pexels.com/photos/6952861/pexels-photo-6952861.jpeg?auto=compress&cs=tinysrgb&w=1600';
        ...
        const transformedSlides = articlesData.map((article) => {
          ...
          return {
            ...
            date: article.published_at ? format(new Date(article.published_at), 'MMMM d, yyyy') : 'Recently published',
    ...
    <section className="relative bg-gray-100 dark:bg-dark-900 py-12">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-serif font-bold text-dark-900 dark:text-white">Featured Stories</h2>
            ...
            {slides.map((slide, index) => (
              <SwiperSlide key={slide.id || index}>
                <article className="group relative overflow-hidden rounded-xl bg-white dark:bg-dark-800">
                  <Link to={`/article/${slide.slug}`} className="block aspect-[4/3]">
                    <img
                      src={slide.image}
                      alt={slide.title}
                      ...
                      onError={(e) => {
                        ...
                        target.src = 'https://images.pexels.com/photos/6952861/pexels-photo-6952861.jpeg?auto=compress&cs=tinysrgb&w=1600';
                      }}
                    />
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6">
                      ...
                      <div className="flex items-center text-sm font-sans text-gray-200">
                        <img
                          src={slide.authorAvatar || 'https://ui-avatars.com/api/?name=Mystic+Banana&background=random'}
                          alt={slide.author}
                          className="w-8 h-8 rounded-full object-cover mr-3"
                          onError={(e) => {
                            const target = e.target as HTMLImageElement;
                            target.src = 'https://ui-avatars.com/api/?name=Mystic+Banana&background=random';
                          }}
                        />
                        <div className="flex flex-col">
                          <span className="font-medium">By {slide.author}</span>
                          <span className="text-gray-300 dark:text-dark-300 text-xs">{slide.date}</span>
                        </div>
                      </div>
                    </div>
                  </Link>
        ...
        {!loading && !error && slides.length === 0 && (
          <div className="bg-dark-800 rounded-xl p-10 text-center">
            <h3 className="text-xl font-medium text-white mb-2">No Featured Articles</h3>
            <p className="text-gray-400 mb-6">There are currently no featured articles to display.</p>
            <Link 
              to="/articles" 
