...
import { generateWebsiteSchema, generateOrganizationSchema, combineStructuredData } from '../utils/structuredData';
...
const SEO: React.FC<SEOProps> = ({
  ...
  const siteUrl = 'https://mysticbanana.com';
  ...
  const finalJsonLd = jsonLd 
    ? (baseSchemas.length > 0 ? combineStructuredData(...baseSchemas, jsonLd) : jsonLd)
    : (baseSchemas.length > 0 ? combineStructuredData(...baseSchemas) : undefined);
  ...
  return (
    <Helmet>
      ...
      {canonicalUrl && <link rel="canonical" href={canonicalUrl.startsWith('http') ? canonicalUrl : `${siteUrl}${canonicalUrl}`} />}
      ...
      {canonicalUrl && <meta property="og:url" content={canonicalUrl.startsWith('http') ? canonicalUrl : `${siteUrl}${canonicalUrl}`} />}
